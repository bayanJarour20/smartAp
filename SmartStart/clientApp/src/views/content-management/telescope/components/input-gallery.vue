  ...
  <b-row>
    <b-col cols="3">
      <div class="position-relative mb-1">
        ...
        <label :for="'label-for-file-' + id + (mul ? '-mul' : '-single')">{{
          label
        }}</label>
        <label
        ...
        <small
          ...
          v-if="
            required &&
            ((mul && (!previewBase || (previewBase && !previewBase.length))) ||
              (!mul && !previewBase))
          "
    ...
    <b-col cols="3" v-for="(src, index) in previewBase" :key="index">
      <div
        ...
        :style="(isChoose == index && typeListDescription == 1) ? 'border: #6610f2; border-style: solid;' : ''"
      >
        <img class="image multie-image" :src="src" style="" width="100%" />
      </div>
    </b-col>
  </b-row>
</template>
...
export default {
  ...
  methods: {
    ...mapMutations(["UPDATE_DOCUMENT_INDEX" , "UPDATE_TYPE_LIST_DESCRIPTION"]),
    clear() {
      this.innerVal = null;
  ...
  watch: {
    innerVal(v) {
      var documents = [];
      if(v != null){
        v.forEach(element => documents.push({ file: element , note: "" , path: ""}))
      } 
