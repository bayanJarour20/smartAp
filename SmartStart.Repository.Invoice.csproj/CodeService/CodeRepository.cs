...
namespace SmartStart.Repository.Invoice.CodeService
{
    ...
    public class CodeRepository : ElRepository<SmartStartDbContext, Guid, Code>, ICodeRepository
    {
        ...
        public async Task<OperationResult<bool>> ActivateCodeV2(string hash, IEnumerable<Guid> subjectFacultyIds)
            => await RepositoryHandler(_activateCodeV2(hash, subjectFacultyIds));
        ...
        private Func<OperationResult<bool>, Task<OperationResult<bool>>> _activateCodeV2(string hash, IEnumerable<Guid> subjectFacultyIds)
            => async operation =>
            {
                ...
                if ((!subjectFacultyIds?.Any()) ?? true)
                    return operation.SetFailed($"Most set {nameof(subjectFacultyIds)}");

                if (subjectFacultyIds.Count() > 4 && one.Type == CodeTypes.Optional)
                    return operation.SetFailed($"{nameof(subjectFacultyIds)} unvalid contain more 4 with Code Type {CodeTypes.Optional} value  {(int)CodeTypes.Optional} ");

                one.UserId = Context.CurrentUserId;
                one.DateActivated = DateTime.Now.ToLocalTime();

                var subjectFaculty = await _query<PackageSubjectFaculty>().Where(s => subjectFacultyIds.Contains(s.SubjectFacultyId))
                                                                          .Select(s => new { s.SubjectFacultyId, s.Price }).ToListAsync();
                ...
                one.CodePackages = new List<CodePackage>() {
                     new CodePackage(){
                         Package = new Package(){
                            ...
                            PackageSubjectFaculties = subjectFaculty.Select(x => new PackageSubjectFaculty 
                            {
                                Price = x.Price,
                                SubjectFacultyId = x.SubjectFacultyId
                            }).ToList()
                         }
                     },
                };

                Context.AddRange(subjectFacultyIds.Except(await _query<SubjectFacultyAppUser>().Where(s => s.AppUserId == Context.CurrentUserId)
                                                                                               .Select(s => s.SubjectFacultyId).ToListAsync())
                                                  .Select(s => new SubjectFacultyAppUser
                                                  {
                                                      SubjectFacultyId = s,
                                                      AppUserId = Context.CurrentUserId.Value,
        ...
        private Func<OperationResult<CodeDetailsDto>, Task<OperationResult<CodeDetailsDto>>> _generate(CodeGenerateDto dto)
            => async operation =>
            {
                ...
                var sellerName = await _query<AppUser>().Where(seller => seller.Id == dto.SellerId 
                                                                      && seller.Type == UserTypes.Seller 
                                                                      && !seller.DateBlocked.HasValue 
                                                                      && !seller.DateDeleted.HasValue)
                                                        .Select(seller => seller.Name).FirstOrDefaultAsync();
